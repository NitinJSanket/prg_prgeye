...
from Network.EVHomographyNetUnsupOnlyTranslationFlow import EVHomographyNetUnsupOnlyTranslationFlow
...
def LossFunc(I1PH, I2PH, MaskPH, AllPtsPH, LabelPH, prHVal, MiniBatchSize, LossFuncName, TrainingType, OriginalImageSize, a=None, c=1e-1, ImageSize=None):
    ...
    prHVal = tf.expand_dims(prHVal, 2)
    prHVal = tf.expand_dims(prHVal, 3)
    prHVal = tf.image.resize_bilinear(prHVal, out_size)
    ...
    # WarpI1 = tf.contrib.image.translate(I1PH, prHVal, interpolation='BILINEAR')
    # WarpMask = tf.contrib.image.translate(MaskPH, prHVal, interpolation='BILINEAR')
    WarpI1 = tf.contrib.image.dense_image_warp(I1PH, prHVal)
    WarpMask = tf.contrib.image.dense_image_warp(MaskPH, prHVal)

    theta = tf.convert_to_tensor(np.float32([[0], [0]]), dtype='float')
    out_size = [MiniBatchSize, OriginalImageSize[0], OriginalImageSize[1], OriginalImageSize[2]]
    I1LP = ptn.polar_transformer(I1PH, theta, out_size, name='I1LP', \
                  log=True, radius_factor=0.5)
    I2LP = ptn.polar_transformer(I2PH, theta, out_size, name='I2LP', \
                  log=True, radius_factor=0.5)
    input('qqq')
...
def TrainOperation(ImgPH, I1PH, I2PH, MaskPH, AllPtsPH, LabelPH, TrainNames, TestNames, NumTrainSamples, ImageSize, Rho,
    ...
    if(NetworkType == 'Small'):
        ...
        prHVal = EVHomographyNetUnsupOnlyTranslationFlow(ImgPH, ImageSize, MiniBatchSize)
        with tf.name_scope('Loss'):
    	    loss, WarpI1, WarpMask, WarpI1Patch, I2Patch, _ = LossFunc(I1PH, I2PH, MaskPH, AllPtsPH, LabelPH, prHVal, MiniBatchSize, LossFuncName, TrainingType, OriginalImageSize, ImageSize=ImageSize)
...
def main():
    ...
    LabelPH = tf.placeholder(tf.float32, shape=(MiniBatchSize, 2, 1), name='Label')  
