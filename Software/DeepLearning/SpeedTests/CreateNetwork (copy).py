...
def SpeedTestModel(ImgPH, ImageSize, CheckPointPath, ModelPrefix, NetworkType, MiniBatchSize, TestMultipleMiniBatch, NumTest, MiniBatchSizeIncrement, MiniBatchMin, Append=False):
    ...
    for count2 in range(StartIdx, MiniBatchSize, MiniBatchSizeIncrement):
        ...
        if(not Append):
            ...
            LogFile = open(CheckPointPath + os.sep + ModelPrefix + 'Log.txt', 'wa')
...
def main():
    ...
    if(Mode == 'Train'):
        ...
        if(ForceBatchSize1):
            SpeedTestModel(ImgPH, ImageSize, CheckPointPath, ModelPrefix, NetworkType, 1, False, NumTest, MiniBatchSizeIncrement, 1, Append=True)
    else:
